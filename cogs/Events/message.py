    ...
    async def on_ready(self):
        if not os.path.exists(self.voice_events_path):
            try:
                ...
                canal_notificacion = self.bot.get_channel(self.canal_notificacion_id)
                if canal_notificacion:
                    await canal_notificacion.send(f"Se creó el archivo `{self.voice_events_path}` en la carpeta `{self.data_folder}`.")
                    await canal_notificacion.send(f"Este archivo almacenará eventos de voz.")
                voice_events_channel = self.bot.get_channel(self.voice_events_channel_id)
                if voice_events_channel:
                    await voice_events_channel.send(f"Se creó el archivo `VoiceEvents.json` en la carpeta `{self.data_folder}`.")
                    await voice_events_channel.send(f"Este archivo almacenará eventos de voz.")
            except Exception as e:
                embed = discord.Embed(
                    title="Error",
                    description=f"Error al crear el archivo JSON: ```{e}```",
                    color=self.color_pastel
                )
                await self.bot.get_channel(self.canal_error_id).send(embed=embed)
